...
public interface TypeRepository extends CrudRepository<Type, Integer> {
	...
	@Query("SELECT DISTINCT t FROM  Book b, BookName bn, Languages l, Type t WHERE bn.id=b.name AND t.id=b.type AND l.id=b.languages AND bn.id= :bookNameId AND l.id= :langId" )
	public Optional<List<Type>> getAllTypesForBookNameAndLanguageFromAllBooks(@Param("bookNameId") Integer bookNameId,@Param("langId") Integer langId);
}
